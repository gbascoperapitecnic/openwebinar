...
export default function PokemonCard({pokemon, handleSelected, handleSelected2}) {

    // es posible que se trate de acceder a una propiedad que no existe (ya que la llamada a la api tardará un tiempo)
    return (
        pokemon.id ? (
            <div className="flex flex-col justify-center items-center p bg-teal-800 text-white rounded-md p-3 cursor-pointer border-black border-2 hover:bg-slate-400 transition-all" onClick={() => handleSelected(pokemon)} onAuxClick={() => handleSelected2(pokemon)}>
