    ...
    <div className="rounded-lg p-2 bg-black/60 mt-3">
        {providersArr.map((provider) => 
            <div key={provider.link} className="p-1 rounded-md">
                <h3 className="text-xl">{changeTitle(provider[0])}</h3>

                <div className="flex items-center gap-3 flex-wrap">
                    {
                        typeof provider[1] === "string" ? (
                            <a href={provider[1]} className="rounded-full bg-indigo-900 px-5 py-2 my-3" target="_blank">Más info</a>
                        ) : (
                            provider[1].map((providerInfo) => 
                                <img src={`https://image.tmdb.org/t/p/original/${providerInfo.logo_path}`} alt="logo-provider" className="w-14 mt-2 rounded-lg"/>
                            )
                        )
                    }
                </div>
